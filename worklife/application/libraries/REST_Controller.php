...
abstract class REST_Controller extends CI_Controller {
    ...
    public $superAdminUsername = 'admin';
    ...
    public function getAuthAdminUserArray() {
        $userArray = $this->getAuthUserArray();
        if ($userArray['permission'] <= 0) {
            $this->responseError(401, 'not admin user');
        }

        return $userArray;
    }

    public function getAuthUserArray() {
        $this->load->model('User_model');
        $userArray = $this->User_model->findUserWithUsername($this->auth_user_username);
        if ($userArray === FALSE) {
    ...
    //permission = 0  : super super admin
    //permission = 1  : common user login
    //permission = 2  : other user maybe also admin 
    ...
    public function HTTPBaseAuth() {
        if (!$this->basicAuthProcessHeader()) {
            $this->responseError(401, $_SERVER);
        }
    ...
    function checkAuthFromDB() {
        $db_lechuang = $this->load->database('default', TRUE);

        $db_lechuang->where('username', $this->auth_user_username);
        $db_lechuang->where('password', $this->auth_user_password);
        $query = $db_lechuang->get('user');
        $db_lechuang->close();
    ...
    function checkAdminUser() {
        $this->sessionAuth();
        $this->getAuthAdminUserArray();
    }

    public function responseError($httpCode, $message = '') {
        $this->response(array('result' => 'error', 'message' => $message), $httpCode);
    }
    ...
    public function responseSuccess($message = '') {
        $this->response(array('result' => 'success', 'message' => $message), 200);
    }

    public function responseBool($bResult) {
        return $bResult === FALSE ? $this->responseError(500) : $this->responseSuccess();
    }
    ...
    function twitterSimpleStatusArray($notice, $include_user = true) {
        ...
        if ($include_user && $user) {
            ...
            $twitter_user = $this->twitterUserArray($user, false);
    ...
    function twitterUserArray($user, $get_notice = false) {
        $twitter_user = array();

        $twitter_user['id'] = intval($user->id);
        $twitter_user['name'] = $user->nickname;
        $twitter_user['profile_image_url'] = $user->avatar_url;
